...
Emacs asks permission to run such a command every time.

Let's disable the command that asks for permission to run a disabled command;
thus enabling all the commands that are disabled by default 😁

#+begin_src emacs-lisp
(setq disabled-command-function 'ignore)
#+end_src

For the sake of curiousity, let's remember the exact list of disabled commands.
...
Add/Change/Delete pairs based on [[https://github.com/magnars/expand-region.el][expand-region]]. Simlar to =evil-surround=, a port of =vim-surround=.
...
Track Emacs commands frequency stats.
...
SLY is a fork of SLIME, feature-rich Common Lisp interaction environment.
